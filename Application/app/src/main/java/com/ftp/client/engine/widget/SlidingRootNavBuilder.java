package com.ftp.client.engine.widget;
...
import com.ftp.client.engine.widget.callback.DragListener;
import com.ftp.client.engine.widget.callback.DragStateListener;
import com.ftp.client.engine.widget.transform.CompositeTransformation;
import com.ftp.client.engine.widget.transform.ElevationTransformation;
import com.ftp.client.engine.widget.transform.RootTransformation;
import com.ftp.client.engine.widget.transform.ScaleTransformation;
import com.ftp.client.engine.widget.transform.YTranslationTransformation;
import com.ftp.client.engine.widget.util.ActionBarToggleAdapter;
import com.ftp.client.engine.widget.util.DrawerListenerAdapter;
import com.ftp.client.engine.widget.util.HiddenMenuClickConsumer;
...
public class SlidingRootNavBuilder {
    ...
    private ViewGroup getContentView() {
        ...
        if (contentView.getChildCount() != 1) {
            throw new IllegalStateException(activity.getString(R.string.engine_ex_bad_content_view));
        }
        return contentView;
    }
    ...
    private View getMenuViewFor(SlidingRootNavLayout parent) {
        if (menuView == null) {
            if (menuLayoutRes == 0) {
                throw new IllegalStateException(activity.getString(R.string.engine_ex_no_menu_view));
            }
    ...
    protected void initToolbarMenuVisibilityToggle(final SlidingRootNavLayout sideNav, View drawer) {
        if (toolbar != null) {
            ...
            ActionBarDrawerToggle toggle = new ActionBarDrawerToggle(activity, dlAdapter, toolbar,
                    R.string.engine_drawer_open,
                    R.string.engine_drawer_close);
            toggle.syncState();
